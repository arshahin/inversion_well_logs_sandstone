function [resist,ffac,rw,logr]=sdem_resist_sandstone(phit,phii,phim,swi,swm,lamdai,lamdam,tmpc,salppk,wetcase)
...
%%%%%%%%%%%%%%%%%%%% Input
%%%%%% phii,phim: Intergranular and micro porosities where phit=phii+phim
%%%%%% swi and swm are intergranular and micorpore water saturations where
%%%%%% swt.phit=swi.phii+phim
...
phiiw=phii.*swi+eps; 
swt=phiiw./phit+swm+eps;
phit_swt=phit.*swt+eps;

if wetcase==1  %%%% water wet 
    ffac=((1./phiiw).^lamdai).*((phiiw./phit_swt).^lamdam); 
    resist=rw.*ffac;    
    logr=1;
elseif wetcase==2  %%%%% oil wet 
      nv=lamdam;
      ni=lamdai+(lamdai-lamdam).*(phim./phii);
      ffac1=((1./phii).^lamdai).*((phii./phit).^lamdam);
