...
lamdai_max=2.00;     lamdai_min=1.5;     dlamdai=0.005;       nlamdai=round(abs(lamdai_max-lamdai_min)./dlamdai);
Cx_max=140.0;        Cx_min=100.0;          dCx=1;              nCx=round(abs(Cx_max-Cx_min)./dCx);
Lik_max=0.30;        Lik_min=0.10;        dLik=0.01;          nLik=round(abs(Lik_max-Lik_min)./dLik);
Limu_max=0.30;       Limu_min=0.10;       dLimu=0.01;         nLimu=round(abs(Limu_max-Limu_min)./dLimu);
phic_max=0.55;       phic_min=0.35;        dphic=0.01;         nphic=round(abs(phic_max-phic_min)./dphic);
ks_max=45.00;         ks_min=25.00;        dks=0.1;            nks=round(abs(ks_max-ks_min)./dks);
mus_max=45.00;         mus_min=25.00;      dmus=0.1;           nmus=round(abs(mus_max-mus_min)./dmus);
rhos_max=2.65;         rhos_min=2.55;       drhos=0.01;        nrhos=round(abs(rhos_max-rhos_min)./drhos);

%%%%%%%%%%%%%% Minimum range of global parameters only for QC
% % lamdai_max=1.76;     lamdai_min=1.74;     dlamdai=0.005;       nlamdai=round(abs(lamdai_max-lamdai_min)./dlamdai);
% % Cx_max=121.0;       Cx_min=119.0;          dCx=1;              nCx=round(abs(Cx_max-Cx_min)./dCx);
% % Lik_max=0.21;        Lik_min=0.19;        dLik=0.01;          nLik=round(abs(Lik_max-Lik_min)./dLik);
% % Limu_max=0.21;       Limu_min=0.19;       dLimu=0.01;         nLimu=round(abs(Limu_max-Limu_min)./dLimu);
% % phic_max=0.46;       phic_min=0.44;        dphic=0.01;         nphic=round(abs(phic_max-phic_min)./dphic);
% % ks_max=35.00;         ks_min=34.00;        dks=0.1;            nks=round(abs(ks_max-ks_min)./dks);
% % mus_max=34.00;         mus_min=33.00;      dmus=0.1;           nmus=round(abs(mus_max-mus_min)./dmus);
% % rhos_max=2.62;         rhos_min=2.60;       drhos=0.01;        nrhos=round(abs(rhos_max-rhos_min)./drhos);
...
% % % % % %         %%%%% plotting of geophysical prospecting paper 
        ...
        for jz=nsh:nz-nsh
            ...
            while logm==-1
                ...
                if (phim_mod(jz)< 0.0 ||  swm_mod(jz)<0.0 ||  swm_mod(jz)> 1.0 ||  swi_mod(jz)< 0.0 ||  swi_mod(jz)> 1.0)
                    logm=-1;
                else
                    logm=1;
                end
        ...
        [emod,logr,logk,logmu]=objfun_sdem_density_sonic_resist_sandstone_v01...
            (resist(nsh:nz-nsh),dtco(nsh:nz-nsh),dtsm(nsh:nz-nsh),den(nsh:nz-nsh),phi_sh,phit_mod(nsh:nz-nsh),...
                               ...
                               swt_mod(nsh:nz-nsh),sot_mod(nsh:nz-nsh),sgt_mod(nsh:nz-nsh),satype,kw,k_oil,k_gas,rhow,rho_oil,rho_gas,rho_clay,k_clay,mu_clay,rhos_mod,ks_mod,mus_mod);   
       
       %%%%%% compute local error  and Initialize local temperatures 
        if wloc~=0
            for jz=nsh:nz-nsh
                
                 [emod_local(jz,1:npar),logr,logk,logmu]=objfun_sdem_density_sonic_resist_sandstone_v01(resist(jz),dtco(jz),dtsm(jz),den(jz),phi_sh,phit_mod(jz),...
                 phic_mod,phii_mod(jz),phim_mod(jz),swi_mod(jz),swm_mod(jz),lamdai_mod,lamdam,tmpc,Cx_mod,wetcase,...
                               Lik_mod,Limu_mod,Lmk,Lmmu,...
                               swt_mod(jz),sot_mod(jz),sgt_mod(jz),satype,kw,k_oil,k_gas,rhow,rho_oil,rho_gas,rho_clay,k_clay,mu_clay,rhos_mod,ks_mod,mus_mod);   
            ...
            for jmov=1:nmov
                ...
                for jz=nsh:nz-nsh
                       ...
                       while logw==-1
                           ...
                           if (phim_trial(jz) < 0.0 ||  swm_trial(jz)<0.0 ||  swm_trial(jz)> 1.0 ||  swi_trial(jz)< 0.0 ||  swi_trial(jz)> 1.0)
                               logw=-1;
                           else
                               logw=1;
                           end
                ...
                [etrial,logr,logk,logmu]=objfun_sdem_density_sonic_resist_sandstone_v01...
                    (resist(nsh:nz-nsh),dtco(nsh:nz-nsh),dtsm(nsh:nz-nsh),den(nsh:nz-nsh),phi_sh,phit_trial(nsh:nz-nsh),...
                               ...
                               swt_trial(nsh:nz-nsh),sot_trial(nsh:nz-nsh),sgt_trial(nsh:nz-nsh),satype,kw,k_oil,k_gas,rhow,rho_oil,rho_gas,rho_clay,k_clay,mu_clay,rhos_trial,ks_trial,mus_trial);
...
% % %                  if (wloc~=0 && jtemp >= iter_loc)
                     for jz=nsh:nz-nsh
                        ...
                        [etrial_local(jz,1:npar),logr,logk,logmu]=objfun_sdem_density_sonic_resist_sandstone_v01...
                            (resist(jz),dtco(jz),dtsm(jz),den(jz),phi_sh,phit_trial(jz),...
                         phic_trial,phii_trial(jz),phim_trial(jz),swi_trial(jz),swm_trial(jz),lamdai_trial,lamdam,tmpc,Cx_trial,wetcase,...
                         Lik_trial,Limu_trial,Lmk,Lmmu,...
                         swt_trial(jz),sot_trial(jz),sgt_trial(jz),satype,kw,k_oil,k_gas,rhow,rho_oil,rho_gas,rho_clay,k_clay,mu_clay,rhos_trial,ks_trial,mus_trial);   
    ...
    for jrun=1:nruns
        [dtp_sim_ppd(:,jrun),dts_sim_ppd(:,jrun),vp_sim_ppd(:,jrun),vs_sim_ppd(:,jrun),ai_sim_ppd(:,jrun),si_sim_ppd(:,jrun)...
            ,vr_sim_ppd(:,jrun),pr_sim_ppd(:,jrun),ksat_sim_ppd(:,jrun),musat_sim_ppd(:,jrun),den_sim_ppd(:,jrun),logk,logmu]=...
             sdem_sonic_sandstone_v01...
             (phi_sh,phit_opt(jrun,:),phii_opt(jrun,:),phim_opt(jrun,:),phic_opt(jrun),Lik_opt(jrun),Limu_opt(jrun),Lmk,Lmmu,...
             swi_opt(jrun,:),swt_opt(jrun,:),sot_opt(jrun,:),sgt_opt(jrun,:),satype,kw,k_oil,k_gas,rhow,rho_oil,rho_gas,rho_clay,k_clay,mu_clay,rhos_opt(jrun),ks_opt(jrun),mus_opt(jrun));
...
[dtp_sim_smth,dts_sim_smth,vp_sim_smth,vs_sim_smth,ai_sim_smth,si_sim_smth,vr_sim_smth,pr_sim_smth,ksat_sim_smth,musat_sim_smth,den_sim_smth,logk,logmu]=...
         sdem_sonic_sandstone_v01...
         (phi_sh,phit_smth,phii_smth,phim_smth,phic_mean,...
         Lik_mean,Limu_mean,Lmk,Lmmu,...
         swi_smth,swt_smth,sot_smth,sgt_smth,satype,kw,k_oil,k_gas,rhow,rho_oil,rho_gas,rho_clay,k_clay,mu_clay,rhos_mean,ks_mean,mus_mean);
